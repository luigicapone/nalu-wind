...
ShearStressTransportEquationSystem::solve_and_update()
{
  ...
  //optimize otherwise computing twice same thing at the init
  // FIXME: Push to geometry algorithm? DES distance if mesh motion is active
   else if ( (SST_DES == realm_.solutionOptions_->turbulenceModel_ || ABL_DES_BLEND == realm_.solutionOptions_->turbulenceModel_) && realm_.solutionOptions_->meshMotion_) {                                                     
    sstMaxLengthScaleAlgDriver_->execute();
